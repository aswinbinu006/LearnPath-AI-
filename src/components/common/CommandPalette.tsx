      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 sm:pt-28 px-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-xl bg-slate-950 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col z-10"
            role="dialog"
            aria-modal="true"
          >
            {/* Search Input Bar */}
            ...
            <div className="max-h-80 overflow-y-auto p-2 space-y-1 select-none font-sans">
              {filteredItems.length > 0 ? (
                ...
                <div className="p-8 text-center text-xs text-slate-500 space-y-1">
                  <Command className="w-6 h-6 mx-auto text-slate-600" />
                  <p className="font-semibold text-slate-400">No matching commands</p>
                  <p>Try searching for "Dashboard", "AI Mentor", or "Theme".</p>
                </div>
              )}
            </div>

            {/* Footer Help */}
            <div className="px-4 py-2 bg-slate-900/40 border-t border-slate-800/60 flex items-center justify-between text-[10px] text-slate-500 font-mono select-none">
