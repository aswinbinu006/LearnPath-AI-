      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto overscroll-contain pb-safe">
          ...
          <motion.div
            ...
            exit="exit"
            className={cn(
              'relative w-full max-h-[90dvh] flex flex-col bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden z-10',
              maxWidthStyles[maxWidth]
            )}
          >
            {/* Header */}
            {(title || subtitle) && (
              <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-100 flex items-start justify-between shrink-0">
                ...
                <button
                  onClick={onClose}
                  className="text-slate-400 hover:text-slate-600 p-2 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer min-h-[40px] min-w-[40px] flex items-center justify-center shrink-0 active:scale-95"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
